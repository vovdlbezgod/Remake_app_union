...
public class BrushDrawingView extends View {
    ...
    public Bitmap maskBrushing(Bitmap src, Bitmap mainWindow){
        Bitmap result = Bitmap.createBitmap(mainWindow.getWidth(), mainWindow.getHeight(), Bitmap.Config.ARGB_8888);
        Paint brushPaint = getmDrawPaint();
        List<LinePath> brushPath = getmLinePaths();
        Canvas brushCanvas = new Canvas(result);
        brushCanvas.drawColor(Color.TRANSPARENT);
        for (LinePath linePath : brushPath) {
            brushCanvas.drawPath(linePath.getDrawPath(), linePath.getDrawPaint());
        }
        brushCanvas.drawPath(getmPath(), brushPaint);
        return Bitmap.createScaledBitmap(result, src.getWidth(), src.getHeight(), false);
    }
    ...
    public List<LinePath> getmLinePaths() {
        return mLinePaths;
    }
